...
char determine_action(Hand* player_hand, int dealer_upcard, char (*strategy_array)[STRAT_COLS],
                      Metadata* metadata) {
    int player_total = get_hand_value(player_hand);
    int row_index;

    if (can_split(player_hand, metadata)) {
        int value = player_hand->cards[0];  // Store the card that we have two of
        // For splitable hands, decisions start at row #28 and index 27 (zero
        // index for array) The row index will stay at 27 for a deuce, and
        // increase by one for every value we increase, so we subtract 2 and add
        // the value of the single card
        row_index = 27 + value - 2;

        return strategy_array[row_index][dealer_upcard - 2];
    } else if (has_soft_ace(player_hand)) {
        // For soft hands, decisions start at row #19 and index 18 (zero index
        // for array) Totals start at soft 13 so we subtract 13
        row_index = 18 + player_total - 13;
        return strategy_array[row_index][dealer_upcard - 2];
    } else {
        // Player totals start at 4 (index 0) and end at 21 (index 17), so if
        // total is 4, the index is 0, so we subtract 4
