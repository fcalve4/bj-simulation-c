...
void play_hand(Shoe* shoe, Hand* player_hand, Hand* dealer_hand, char (*strategy)[STRAT_COLS],
               Metadata* metadata) {
    ...
    if (metadata->enhc == 0) {
        // If a natural is returned, return to the start of the loop, else
    ...
    while (player_turn_loop_bool == 1 && get_hand_value(player_hand) < 21) {
        player_turn_loop_bool = play_player_turn(player_hand, dealer_hand, shoe, strategy,
    ...
    // Maybe we can do some work on this so we dont have 2 separate checks for
    // if ehnc == 1. not what i would change tho.
    if (metadata->enhc == 1) {
        // If a natural is returned, return to the start of the loop, else
        // continue play as normal
        if (check_for_naturals(player_hand, dealer_hand, metadata) == 1) {
...
void play_shoe(Hand* player_hand, Hand* dealer_hand, char (*strategy)[STRAT_COLS],
               Metadata* metadata) {
...
void split(Hand* player_hand, Hand* dealer_hand, Shoe* shoe, char (*strategy)[STRAT_COLS],
           int dealer_upcard, int dealer_upcard_value, Metadata* metadata) {
    // Grab the split card
    ...
    while (split_loop_bool) {
        // Call the player turn function for the split off hand and play
        // normally This should just play another hand and then once this loop
        // breaks, the next hand will play via the main loop
        split_loop_bool = play_player_turn(player_hand, dealer_hand, shoe, strategy, dealer_upcard,
...
int play_player_turn(Hand* player_hand, Hand* dealer_hand, Shoe* shoe, char (*strategy)[STRAT_COLS],
                     int dealer_upcard, int dealer_upcard_value, Metadata* metadata) {
...
void play_dealer_turn(Hand* dealer_hand, Shoe* shoe, int h17) {
    ...
    if (h17 == 1) {
        while (get_hand_value(dealer_hand) < 17 ||
               ((has_soft_ace(dealer_hand) == 1) && get_hand_value(dealer_hand) == 17)) {
            add_card_to_hand(dealer_hand, deal_card(shoe));
        }
...
void determine_winner(Hand* player_hand, Hand* dealer_hand, Metadata* metadata) {
...
// HAPPENS TOO
int check_for_naturals(Hand* player_hand, Hand* dealer_hand, Metadata* metadata) {
